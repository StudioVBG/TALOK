/**
 * Email template: amortization-created
 */
export function amortizationcreatedEmail(params: Record<string, string>) {
  ...
  const t = templates["amortization-created"] ?? { subject: "Notification Talok", body: "" };
  const subject = t.subject.replace(/\$\{params\.(\w+)\}/g, (_, k) => params[k] ?? "");
  const body = t.body.replace(/\$\{params\.(\w+)\}/g, (_, k) => params[k] ?? "");
  return {
    subject,
    html: `<div style="font-family:'Manrope',sans-serif;max-width:600px;margin:0 auto;padding:24px"><h2 style="color:#1B2A6B">${subject}</h2><p>Bonjour ${params.userName ?? ""},</p><p>${body}</p><a href="${params.appUrl ?? "https://app.talok.fr"}/owner/accounting" style="display:inline-block;background:#2563EB;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;margin-top:16px">Voir dans Talok</a><p style="margin-top:24px;font-size:12px;color:#999">Talok — Gestion locative simplifiee</p></div>`,
  };
}
