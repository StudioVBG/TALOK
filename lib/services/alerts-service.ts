    ...
    for (const invoice of invoices) {
      ...
      if (shouldAlert) {
        alerts.push({
          ...
          entityType: "invoice",
          entityId: invoice.id,
          actionUrl: `/app/owner/money?invoice=${invoice.id}`,
    ...
    for (const lease of leases) {
      ...
      if (daysRemaining <= 90) {
        ...
        alerts.push({
          ...
          actionUrl: `/app/owner/contracts/${lease.id}`,
    ...
    for (const property of properties) {
      ...
      if (daysRemaining <= 60) {
        alerts.push({
          ...
          entityName: property.adresse_complete,
          actionUrl: `/app/owner/properties/${property.id}?tab=diagnostics`,
    ...
    for (const lease of leases) {
      ...
      if (daysRemaining <= 30 && daysRemaining >= 0) {
        ...
        alerts.push({
          ...
          entityId: lease.id,
          entityName: property?.adresse_complete,
          actionUrl: `/app/owner/contracts/${lease.id}?action=revision`,
