...
export default async function BuildingDetailPage({ params }: PageProps) {
  ...
  if (!property) {
    ...
    if (anyProperty) {
      ...
      console.error("[building-detail] Property not found in DB:", { id });
    }
  ...
  // Fetch units + documents + cover photo en parallèle
  const [unitsResult, documentsResult, coverPhotoResult] = await Promise.all([
    buildingMeta?.id
      ? serviceClient
          .from("building_units")
          .select(`
  ...
  const buildingCoverUrl =
    ((coverPhotoResult.data as { url?: string | null } | null)?.url) ?? null;

  // Fetch lot properties (cover photos via `photos` table, unique_code) and
  // active leases/tenants in parallel. Chaque lot pointe vers son propre
  // property_id — on récupère ici les infos nécessaires pour afficher le lot
  // avec la même card que la page "Mes biens".
  ...
  const [lotPropertiesResult, lotPhotosResult, leasesResult] = await Promise.all([
    lotPropertyIds.length > 0
      ? serviceClient
          .from("properties")
          .select("id, unique_code, adresse_complete")
  ...
  const rawLotProperties = (lotPropertiesResult.data || []) as Array<{
    id: string;
    unique_code: string | null;
    adresse_complete: string | null;
  }>;

  const lotPhotos = (lotPhotosResult.data || []) as Array<{
    property_id: string;
    url: string | null;
  ...
  // Index: première photo par property_id (l'ordre de la query garantit
  // is_main=true en premier, puis ordre croissant).
  const coverByPropertyId = new Map<string, string | null>();
  for (const photo of lotPhotos) {
    if (!coverByPropertyId.has(photo.property_id)) {
      coverByPropertyId.set(photo.property_id, photo.url ?? null);
    }
  }

  const lotProperties = rawLotProperties.map((p) => ({
    ...p,
    cover_url: coverByPropertyId.get(p.id) ?? null,
  }));
  ...
  return (
    <BuildingDetailClient
      ...
      building={{
        ...building,
        cover_url: buildingCoverUrl,
        annee_construction: (building as any).annee_construction ?? null,
      }}
