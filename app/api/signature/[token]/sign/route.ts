...
/**
 * POST /api/signature/[token]/sign — Signature bail via lien + OTP
...
export async function POST(request: Request, { params }: PageProps) {
  ...
  try {
    ...
    const { otp_code, signatureImage } = body;
    if (!otp_code) return NextResponse.json({ error: "Code de vérification requis" }, { status: 400 });
    ...
    const signerName = signer.invited_name || tokenData.email;
    const image = signatureImage || `OTP_VERIFIED:${tokenData.email}`;
    ...
    const result = await executeSignature({
      ...
      signerProfileId: signer.profile_id,
