...
// Créer un client Supabase avec la clé service (lazy pour éviter les erreurs au build)
...
// Client Resend pour l'envoi d'emails (lazy)
...
async function markReminderSent(supabase: ReturnType<typeof getSupabaseAdmin>, reminderId: string) {
  await supabase
    .from("onboarding_reminders")
    .update({
      status: "sent",
...
async function markReminderFailed(supabase: ReturnType<typeof getSupabaseAdmin>, reminderId: string, errorMessage: string) {
  await supabase
    .from("onboarding_reminders")
    .update({
      status: "failed",
...
async function markReminderCancelled(supabase: ReturnType<typeof getSupabaseAdmin>, reminderId: string, reason: string) {
  await supabase
    .from("onboarding_reminders")
    .update({
      status: "cancelled",
