...
/**
 * GET /api/documents/[id]/download — Redirect vers URL signee (SOTA 2026)
 * Corrige le bug DocumentDownloadButton qui envoyait GET sur une route POST-only
 */
export async function GET(
  ...
  try {
    ...
    if (!docData.storage_path) return NextResponse.json({ error: "Document sans fichier" }, { status: 400 });

    const { data: signedUrl, error: urlError } = await supabase.storage.from("documents").createSignedUrl(docData.storage_path, 3600);
    if (urlError || !signedUrl?.signedUrl) return NextResponse.json({ error: "Erreur URL" }, { status: 500 });
