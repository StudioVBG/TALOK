...
/**
 * POST /api/seasonal/sync/airbnb — Sync iCal Airbnb
 * Importe les réservations depuis un feed iCal Airbnb
 */
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      if (!res.ok) throw new ApiError(502, "Impossible de récupérer le calendrier Airbnb");
    ...
    for (const event of events) {
      const reservation = icalEventToReservation(event, listing_id, listing.property_id, "airbnb");
      if (!reservation) {
        skipped++;
        continue;
      }
      ...
      if (insertError) {
        // Overlap or other constraint — skip
        skipped++;
      } else {
        imported++;
      }
