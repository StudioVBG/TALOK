-- ============================================================================
-- MIGRATION: Corriger le filtre de statuts dans tenant_dashboard RPC
-- Date: 2026-02-19
-- Description:
--   Le filtre l.statut IN (...) excluait les baux en cours de signature
--   (partially_signed, pending_owner_signature) et notice_given,
--   rendant le bail invisible au locataire pendant ces étapes.
-- ============================================================================
...
BEGIN
  ...
  --    Suppression du filtre p.role = 'tenant' pour supporter les profils
  --    dont le rôle n'est pas encore confirmé (ex: juste après inscription)
  SELECT p.id, u.email,
         jsonb_build_object(
           'id', p.id,
           'prenom', p.prenom,
  ...
  WHERE p.user_id = p_tenant_user_id;
  ...
  --    FIX: Inclure TOUS les statuts pertinents (partially_signed, pending_owner_signature, notice_given)
...
'RPC dashboard locataire v5. Corrections:
- Suppression du filtre role=tenant (supporte tous les profils)
- Inclut tous les statuts sauf draft/archived (partially_signed, pending_owner_signature, notice_given)
- Cherche par profile_id OU invited_email
