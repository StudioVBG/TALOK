-- ====================================================================
-- Sprint B2 — Phase 1 SAFE — Batch 10/10
-- 1 migrations
--
-- COMMENT UTILISER :
...
-- --------------------------------------------------------------------
-- Migration: 20260417110000_purge_identity_2fa_cron.sql
-- Risk: SAFE
-- Why: Idempotent / structural only
-- --------------------------------------------------------------------
DO $pre$ BEGIN RAISE NOTICE '▶ Applying 20260417110000_purge_identity_2fa_cron.sql'; END $pre$;
...
VALUES ('20260417110000', 'purge_identity_2fa_cron')
ON CONFLICT (version) DO NOTHING;

DO $post$ BEGIN RAISE NOTICE '✓ Applied  20260417110000_purge_identity_2fa_cron.sql'; END $post$;

COMMIT;

-- END OF BATCH 10/10 (Phase 1 SAFE)
