...
> ⚠️ Toutes les estimations de temps sont indicatives. Faire précéder chaque phase d'un snapshot DB (Supabase Dashboard → Database → Backups).
...
- [ ] Cron secret configuré (`ALTER DATABASE postgres SET app.settings.cron_secret = ...`) — prérequis explicite de `20260304100000_activate_pg_cron_schedules.sql`
- [ ] `app.settings.app_url` configuré (idem)
- [ ] Extension pg_cron activée (sinon plusieurs migrations pending échoueront)
- [ ] `TWILIO_VERIFY_SERVICE_SID` configuré dans Netlify (notre migration Sprint 1)
- [ ] Annoncer maintenance window si Phase 4 exécutée (≥ 2h en heures creuses)

## Phase 1 — SAFE batch (🟢 application directe)

**46 migrations** à appliquer en ordre chronologique.
...
<details><summary>Liste complète (cliquer pour déplier)</summary>
...
## Phase 2 — MODÉRÉ batch (🟡 review manuelle)

**76 migrations** à appliquer en ordre chronologique.
...
| 12 | `20260221_fix_owner_data_chain.sql` | UPDATE |
| 13 | `20260221200000_sync_edl_signer_to_lease_signer.sql` | +1 triggers |
...
## Phase 3 — DANGEREUX batch (🔴 un par un, snapshot avant chaque)

**51 migrations** à appliquer en ordre chronologique.
...
## Phase 4 — CRITIQUE (🚨 maintenance window, hors horaires)

**50 migrations** à appliquer en ordre chronologique.
...
## Candidats à squasher (NON exécuté dans ce sprint)

Migrations probablement redondantes, à fusionner/supprimer lors du **Sprint B squash** — candidats détectés par :

1. **Timestamps dupliqués** (2 migrations même seconde)
   → 13 paires. Exemples :
   - `20260224100000` : `20260224100000_fix_tenant_dashboard_notifications_query.sql` + `20260224100000_normalize_provider_names.sql`
   - `20260226000000` : `20260226000000_backfill_existing_invoices_tenant_id.sql` + `20260226000000_fix_notifications_triggers.sql`
   - `20260306100000` : `20260306100000_add_digicode_interphone_columns.sql` + `20260306100000_invoice_on_fully_signed.sql`
   - `20260310200000` : `20260310200000_add_signature_push_franceconnect.sql` + `20260310200000_fix_property_limit_extra_properties.sql`
   - `20260328100000` : `20260328100000_create_site_content.sql` + `20260328100000_fix_visible_tenant_documents.sql`
   → Action : choisir la version "canonique" pour chaque paire, soit en comparant leur contenu, soit en gardant la plus récente par convention (mais attention à l'ordre d'application).

2. **Migrations "fix" qui annulent une précédente**
   Pattern typique dans les noms : `*_fix_*`, `*_rollback_*`. Exemples trouvés dans les pending :
   → 72 fichiers matchent. À examiner individuellement pour identifier les couples migration+fix squashables.

3. **Oscillations ADD/DROP colonne** — à résoudre en fusion sémantique.
   (Cf. `sprint-a-conflicts-report.md` pour la liste complète.)

## Stratégie recommandée

### Option stricte (sécurité max, recommandée)
1. Créer staging Supabase (Option A)
2. Appliquer la totalité en staging (ordre chronologique, 223 migrations)
3. Si tout passe : backup prod + répliquer en prod en 4 phases
4. Après stabilisation (2 semaines) : Sprint B squash des 223 → ~50 consolidées

### Option pragmatique (plus rapide, risque modéré)
1. Shadow DB locale (`supabase start`)
2. Valider la syntaxe (toutes les migrations passent sans erreur)
3. Backup prod + appliquer Phase 1 (SAFE) direct en prod
4. Pour Phases 2-4 : staging dédié obligatoire

### Option urgente (NE PAS RECOMMANDER sauf feu vert Thomas)
1. Backup prod
2. `supabase db push` en prod (toutes les 223 d'un coup)
3. Rollback via restore si échec — perte de toute activité prod du laps de temps
4. ❌ Déconseillé : impossible d'identifier le coupable en cas d'échec cascade
